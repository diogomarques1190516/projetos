...
random_permutation([[t2,t5,t4,t3,t1]*11,
[t2,t1,t4,t5,t3]*13,[t2,t4,t3,t5,t1]*14,
[t2,t5,t3,t4,t1]*14,[t2,t3,t4,t1,t5]*15,
[t2,t1,t5,t4,t3]*16,[t4,t3,t1,t2,t5]*29,
[t1,t4,t5,t2,t3]*34],LRP),nl,write('LRP='),write(LRP),nl.

divide(L,R1,R2):-subdivide(L,S1,R1,_),subdivide(L,S2,R2,_),
S2 is S1+S1+S1,length(L,S),S is S1+S1+S1+S1.
...
dynamic_change():-write('Do you want to insert a dynamic change?(write 1 for yes)'),read(Dyn),
	( Dyn == 1 ->
		write('0 for change load, 1 for remove delivery, 2 for add delivery'), read(Change),
		( Change == 0 ->
		write('Write delivery id and new load'), read(Did), read(Load),
		entrega(Did, Date, _, IWH, TC, TR), retract(entrega(Did,_,_,_,_,_)), asserta(entrega(Did,Date,Load,IWH,TC,TR))
		; Change == 1 ->
		write('Write delivery id'), read(Did), retract(entrega(Did,_,_,_,_,_))
		; Change == 2 ->
		write('Write delivery id, date, load, warehouse, place time and pickup time'), read(Did), read(Date), read(Load), read(IWH), read(TC), read(TR),
		asserta(entrega(Did,Date,Load,IWH,TC,TR))
		), gera2 ; !
	).
